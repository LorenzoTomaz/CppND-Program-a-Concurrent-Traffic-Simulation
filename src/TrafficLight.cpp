...
{   
    ...
    std::mt19937 range(randDev());
    std::uniform_int_distribution<std::mt19937::result_type> dstr(4,6);
    int randSecs = dstr(range);
    int countCycles = 0;

    auto pause = std::chrono::high_resolution_clock::now();
    auto init = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(pause - init);

    std::cout << "Stochastic time: " << randSecs << std::endl;

    while (true) {
        if (countCycles >= 2) {
            pause = std::chrono::high_resolution_clock::now();
            duration = std::chrono::duration_cast<std::chrono::milliseconds>(pause - init);
            if (duration.count()/1000 > randSecs) {
                ...
                // tmp copy to move
                TrafficLightPhase traffPhase = _currentPhase;
                std::future<void> _ftr = std::async(std::launch::async, &MessageQueue<TrafficLightPhase>::send, _msgQueue, std::move(traffPhase));
